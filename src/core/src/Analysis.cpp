...
AnalysisUpdate TokenAnalysisService::FailureUpdate(const GmgnFailure& failure) {
    ...
    return {false, false, retryable, FailureDiagnostic(failure), std::nullopt, retryable ? failure.retryAfter : std::nullopt};
}
AnalysisUpdate TokenAnalysisService::Analyze(const FrozenTokenCluster& cluster, const std::stop_token stop) {
    ...
    { const auto infoPermit = scheduler_.Acquire(GmgnRequestScheduler::Priority::Enrichment, stop); if (!infoPermit) return {false, false, false, "Token enrichment was cancelled before GMGN admission."}; infoResult = client_->FetchTokenInfo(cluster.token, stop); }
    ...
    { const auto securityPermit = scheduler_.Acquire(GmgnRequestScheduler::Priority::Enrichment, stop); if (!securityPermit) return {false, false, false, "Token enrichment was cancelled before GMGN admission."}; securityResult = client_->FetchTokenSecurity(cluster.token, stop); }
...
void TokenAnalysisExecutor::Stop() noexcept { { std::scoped_lock lock(mutex_); accepting_ = false; wake_.notify_all(); } if (worker_.joinable()) { worker_.request_stop(); wake_.notify_all(); worker_.join(); } std::scoped_lock lock(mutex_); pending_.clear(); service_.ClearSession(); }
